...
<body>
    ...
    <div class="header">
        <div class="org">{{ $prbl05->prbl01_created_by_organization_name ?? 'Finance Playground' }}</div>
        <h1>Laporan Bulanan</h1>
        <div class="subtitle">{{ $teamName }} &bull; {{ $bulanLabel }} {{ $tahun }}</div>
        <div class="subtitle">Referensi PP: {{ $ppLabel }}</div>
        <div class="subtitle">Dikompilasi {{ $prbl05->created_at->format('d F Y, H:i') }} WIB</div>
    </div>
