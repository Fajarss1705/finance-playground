...
<body>
    ...
    <div class="header">
        <div class="org">{{ $pabd05->pabd01_created_by_organization_name ?? 'Finance Playground' }}</div>
        <h1>Laporan Pengajuan Anggaran Bulanan</h1>
        <div class="subtitle">{{ $teamName }} &bull; {{ $bulan }} {{ $tahun }}</div>
        @if($ppLabel)
            <div class="subtitle">Referensi PP: {{ $ppLabel }}</div>
        @endif
        <div class="subtitle">Dikompilasi {{ $pabd05->created_at->format('d F Y, H:i') }} WIB</div>
    </div>
