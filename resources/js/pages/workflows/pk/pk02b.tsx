...
export default function Pk02b({
    ...
    const breadcrumbs: BreadcrumbItem[] = scope === 'team'
        ? [
            ...
            { title: 'PK02B: Approval Anggaran', href: '#' },
        ]
        : [
...
function ApproveButton({ basePath }: { basePath: string }) {
    ...
    return (
        <ActionConfirmDialog
            ...
            title="Setujui Anggaran Program Kegiatan"
            description="Anggaran program kegiatan akan disetujui."
            confirmLabel="Setujui"
            processing={processing}
            onConfirm={({ notes, files }) => {
                setProcessing(true);
                router.post(`${basePath}/pk02b/approve`, { notes, ...(files.length > 0 ? { files } : {}) }, {
...
function RejectButton({ basePath }: { basePath: string }) {
    ...
    return (
        <ActionConfirmDialog
            ...
            title="Tolak Anggaran Program Kegiatan"
            ...
            onConfirm={({ notes, files }) => {
                setProcessing(true);
                router.post(`${basePath}/pk02b/reject`, { notes, ...(files.length > 0 ? { files } : {}) }, {
                    forceFormData: files.length > 0,
                    onFinish: () => setProcessing(false),
                });
            }}
