...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'PRBL-2027-KA-03', href: '/team/workflows/prbl/prbl-uuid-001' },
];
...
export default function TeamPrblShow() {
    return (
                                        ...
                                        <div className="flex shrink-0 items-center gap-2">
                                            ...
                                            {step.status === 'completed' && step.scope === 'team' && (
                                                <Button variant="outline" size="sm" asChild>
                                                    <Link href={`/team/workflows/prbl/prbl-uuid-001/${step.code.toLowerCase()}`}>
                                                        Lihat
                                                    </Link>
                                                </Button>
                                            )}
                                            {step.status === 'completed' && step.scope === 'admin' && (
                                                <span className="text-xs text-muted-foreground">
                                                    {step.code === 'PRBL02A' && 'Disetujui oleh Tim Monev'}
                                                    {step.code === 'PRBL02B' && 'Disetujui oleh Tim Bendahara Umum'}
                                                </span>
                                            )}
                                            {step.status === 'active' && step.scope === 'admin' && (
                                                <span className="text-xs text-muted-foreground">Menunggu review akhir</span>
                                            )}
                                        </div>
                                    </div>
                                ))}
