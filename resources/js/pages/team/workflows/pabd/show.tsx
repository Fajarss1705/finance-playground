...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: workflowKode, href: `/team/workflows/pabd/${workflowUuid}` },
];
...
const stepDetails = [
    { code: 'PABD01', label: 'Pengajuan', status: 'completed' as const, scope: 'team', link: `/team/workflows/pabd/${workflowUuid}/pabd01` },
    { code: 'PABD02A', label: 'Perubahan', status: 'completed' as const, scope: 'team', link: `/team/workflows/pabd/${workflowUuid}/pabd02a` },
    { code: 'PABD02B', label: 'Persetujuan', status: 'active' as const, scope: 'admin', link: null },
    { code: 'PABD03', label: 'Verifikasi', status: 'pending' as const, scope: 'admin', link: null },
    { code: 'PABD04', label: 'Bukti Transfer', status: 'pending' as const, scope: 'team', link: null },
    { code: 'PABD05', label: 'Final', status: 'pending' as const, scope: 'admin', link: null },
];
...
export default function TeamPabdShow() {
    return (
                                ...
                                {stepDetails.map((step) => (
                                    <div key={step.code} className="flex items-center justify-between gap-2 rounded-md border px-3 py-2">
                                        ...
                                        <div className="flex shrink-0 items-center gap-2">
                                            ...
                                            {step.status === 'completed' && step.link ? (
                                                <Button variant="ghost" size="sm" asChild>
                                                    <Link href={step.link}>Lihat</Link>
                                                </Button>
                                            ) : step.status === 'active' && step.scope === 'admin' ? (
